...
First here is an example of a simple code for visualizing the distance between all fractions from a single treatment-control sample pair. In this case, you have a single phyloseq just as in [Chapter_Examples](https://github.com/buckleylab/HR-SIP_example/Chapter_Examples.html). The following uses custom code which can be changed to suit your needs or coding style.

#### 1. Import the data

As with [Chapter_Examples](https://github.com/buckleylab/HR-SIP_example/Chapter_Examples.html) you will use the data found in file "SIP_phyloseq.rds".

```{r, message=FALSE, warning=FALSE}
# Import the data you using the readRDS() function
S1D1.SIP.physeq <- readRDS("SIP_phyloseq.rds")
...
```{r, message=FALSE, warning=FALSE}
# Import the data you using the readRDS() function
S2D2.SIP.physeq <- readRDS("example_S2D2_phyloseq.rds")
